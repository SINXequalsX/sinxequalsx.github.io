...
function validateContent(value) {
  ...
  for (const {id:key} of navigation) {
    ...
    output[key] = { title:cleanText(page.title,200), intro:cleanText(page.intro,1000), backgroundImage:cleanText(page.backgroundImage,1000), blocks:page.blocks.slice(0,100).map((block,index) => ({
      ...
      eyebrow:cleanText(block.eyebrow,300), title:cleanText(block.title,500), body:cleanText(block.body), meta:cleanText(block.meta,500), imageSrc:cleanText(block.imageSrc,1000), imageAlt:cleanText(block.imageAlt,500), imageHeight:Math.min(900,Math.max(220,Number(block.imageHeight) || 400)),
