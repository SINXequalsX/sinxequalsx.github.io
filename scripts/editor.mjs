...
function validateContent(value) {
  ...
  for (const {id:key} of navigation) {
    ...
    output[key] = { title:cleanText(page.title,200), intro:cleanText(page.intro,1000), backgroundImage:cleanText(page.backgroundImage,1000), blocks:page.blocks.slice(0,100).map((block,index) => ({
      id:cleanText(block.id,100) || `${key}-${index}`, type:types.includes(block.type) ? block.type : 'text', tone:tones.includes(block.tone) ? block.tone : 'white', size:sizes.includes(block.size) ? block.size : 'full', fontStyle:fonts.includes(block.fontStyle) ? block.fontStyle : 'clean', textColor:textColors.includes(block.textColor) ? block.textColor : 'default', titleSize:Number(block.titleSize) ? Math.min(180,Math.max(32,Number(block.titleSize))) : 0,
      eyebrow:cleanText(block.eyebrow,300), title:cleanText(block.title,500), body:cleanText(block.body), meta:cleanText(block.meta,500), imageSrc:cleanText(block.imageSrc,1000), imageAlt:cleanText(block.imageAlt,500), imageHeight:Math.min(900,Math.max(220,Number(block.imageHeight) || 400)), heroImageSize:Math.min(600,Math.max(140,Number(block.heroImageSize) || 300)),
      linkLabel:cleanText(block.linkLabel,200), linkUrl:cleanText(block.linkUrl,250000), pdfLabel:cleanText(block.pdfLabel,200), pdfSrc:cleanText(block.pdfSrc,1000),
      links:Array.isArray(block.links) ? block.links.slice(0,30).map((item,itemIndex) => ({id:cleanText(item.id,100) || `link-${itemIndex}`,label:cleanText(item.label,200),url:cleanText(item.url,250000)})) : [],
