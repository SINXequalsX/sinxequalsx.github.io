...
function validateContent(value) {
  ...
  for (const key of pages) {
    ...
    output[key] = { title:cleanText(page.title,200), intro:cleanText(page.intro,1000), blocks:page.blocks.slice(0,100).map((block,index) => ({
      ...
      eyebrow:cleanText(block.eyebrow,300), title:cleanText(block.title,500), body:cleanText(block.body), meta:cleanText(block.meta,500), imageSrc:cleanText(block.imageSrc,1000), imageAlt:cleanText(block.imageAlt,500), items:Array.isArray(block.items) ? block.items.slice(0,100).map(item => cleanText(item,2000)) : [],
    })) };
  }
  return output;
}
